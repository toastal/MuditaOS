...
namespace gui
{
    ...
    void ThreadWindowOptions::onBeforeShow(gui::ShowMode mode, gui::SwitchData *data)
    {
        ...
        if (threadValid()) {
            ...
            LOG_ERROR("Wchodzisz tutaj ?");
            application->returnToPreviousWindow();
        }
    }

    bool ThreadWindowOptions::threadValid()
    {
        auto number  = DBServiceAPI::NumberByID(application, thread->numberID);
        auto contact = DBServiceAPI::MatchContactByNumberID(application, thread->numberID);

        return !contact->isTemporary();
    }
