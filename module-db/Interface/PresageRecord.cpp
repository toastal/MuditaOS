...
bool PresageRecordInterface::Add(const PresageRecord &rec) {

    uint32_t contactID = 0;

    // Create alarm
    presageDB->presage.Add(PresageTableRow{
    		.time = rec.time,
    		.snooze = rec.snooze,
			.status = rec.status,
			.path = rec.path
...
std::unique_ptr<std::vector<PresageRecord>> PresageRecordInterface::GetLimitOffsetByField(uint32_t offset, uint32_t limit,
...
std::unique_ptr<std::vector<PresageRecord>> PresageRecordInterface::GetLimitOffset(uint32_t offset, uint32_t limit) {
...
bool PresageRecordInterface::Update(const PresageRecord &rec) {
...
PresageRecord PresageRecordInterface::GetByID(uint32_t id) {
    auto alarm = presageDB->presage.GetByID(id);
...
PresageRecord PresageRecordInterface::GetNext(time_t time) {
    auto alarm = presageDB->presage.GetNext(time);
