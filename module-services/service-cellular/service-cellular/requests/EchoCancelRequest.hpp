...
namespace cellular
{
    ...
    class EchoCancelRequest : public Request
    {
      public:
        EchoCancelRequest(const std::string &data) : Request(data){};
        std::string command() final;

        static std::unique_ptr<EchoCancelRequest> create(const std::string &data, GroupMatch);
